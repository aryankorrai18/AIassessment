...
export function accessKeyEmail(p: { to: string; name: string; key: string; scheduledAt: Date }): EmailMessage {
...
<tr><td>Email</td><td><code>${escapeHtml(p.to)}</code></td></tr>
...
export function reminderEmail(p: { to: string; name: string; scheduledAt: Date; reminderNumber: number }): EmailMessage {
  const url = candidatePortalUrl();
  const when = formatWhen(p.scheduledAt);
  const text = [
    `Hi ${p.name},`,
...
<p>Sign in at <a href="${escapeHtml(url)}">${escapeHtml(url)}</a> with this email address (<code>${escapeHtml(p.to)}</code>) and the access key from your original invitation email.</p>
...
export function noShowEscalationEmail(p: { to: string; candidateName: string; candidateEmail: string; refId: string | null; scheduledAt: Date }): EmailMessage {
  const who = p.refId ? `${p.candidateEmail}, ref ${p.refId}` : p.candidateEmail;
  const when = formatWhen(p.scheduledAt);
  const text = [
    `${p.candidateName} (${who}) has been marked NO_SHOW.`,
    "",
  ...
  const html = `<p><strong>${escapeHtml(p.candidateName)}</strong> (${escapeHtml(who)}) has been marked <strong>NO_SHOW</strong>.</p>
...
<p>Reschedule or re-send their key from Admin → Interview Schedule if they should still be assessed.</p>`;
  return { to: p.to, subject: `No-show: ${p.candidateName} (${p.candidateEmail})`, html, text };
}
