...
export function completeInTransaction(tx: Transaction, interviewRef: DocumentReference<InterviewDoc>, email: string) {
  ...
  tx.set(completedInterviewsCol().doc(email), { completedAt: now });
}
...
export async function buildCandidateProfile(email: string, candidate: CandidateDoc, interview: InterviewDoc): Promise<CandidateProfile> {
  ...
  return { email, name: candidate.name, refId: candidate.refId ?? null, cluster: candidate.skillCluster, jdRef, hasCoding: jd?.requiresCoding ?? false };
}
